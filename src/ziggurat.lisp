...
(defun tail-integrate (function r quadrature)
  (letv* (((s w) quadrature))
    (+ (* r (funcall function r))
       (iter (for ii below (length s))
	     (summing (* (aref w ii) (exp (aref s ii)) (funcall function (+ r (aref s ii)))))))))
...
(defun ziggurat-bisect (function r0
			...
			&aux
			  (quadrature (multiple-value-list (u.opoly:gauss-quadrature n-quadrature (u.opoly:orthogonal-polynomial :laguerre))))
			  (rl 0d0) (rh nil))
...
(defun ziggurat-compile (function points v
  ...
  (letv* ((log-nz (floor (log n-ziggurat 2)))
    ...
    (once-only (f0 r0 ktab ytab wtab)
      `(locally
